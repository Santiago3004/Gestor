...
app.config['MAIL_USERNAME'] = 'santiago316504@gmail.com'
app.config['MAIL_PASSWORD'] = 'tzzl pugr wiwd xdzk'
app.config['MAIL_USE_TLS'] = False
app.config['MAIL_USE_SSL'] = True
app.config['MAIL_DEFAULT_SENDER'] = ('Servisio nacional de aprendizaje sena - cenigraf','santiago316504@gmail.com')
mail = Mail(app)
...
def enviar_correo(email):
   #se genera un toquen para el corrreo proporcionado
   token = serializer.dumps(email, salt='restablecimirento-contraseña')
   #se crea la url
   enlace = url_for('restablecer_contraseña',token=token, _external=True)
   #se crea el mensaje del correo enviado
   mensaje = Message(subject='Restableciimiento de contraseña',recipients=[email],body=f'Para restablecer contraseña, click en el siguiente enlace:{enlace}')
...
def restablecer_contraseña(token):
   ...
   if request.method == 'POST':
      ...
      cursor = db.cursor()
      email = serializer.loads(token, salt='restablecimirento-contraseña', max_age=3600)
...
def buscar_tarea():
   ...
   # REsalizar la busqueda en la base de datos
